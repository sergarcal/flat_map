...
export default function MapCanvas({
  ...
  function DrawManager() {
    ...
    useEffect(() => {
      ...
      const onCreated = (rawEvent: LeafletEvent) => {
        const event = rawEvent as LeafletEvent & {
          layerType: string;
          layer: L.Layer;
        };
        handleCreated(event);
  ...
  const zoneById = useMemo(
    () => new globalThis.Map(zones.map((z) => [z.id, z])),
    [zones],
  );
  ...
  const handleCreated = (
    event: LeafletEvent & { layerType: string; layer: L.Layer },
  ) => {
      ...
      <FeatureGroup ref={featureGroupRef}>
        {zones.map((zone) =>
          ...
          zone.meta?.radiusMeters ? (
            ...
            <Polygon
              key={zone.id}
              positions={zone.geometry.coordinates[0].map(([lng, lat]) => [
                lat,
                lng,
              ])}
              pathOptions={{
